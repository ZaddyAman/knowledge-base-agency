...
function ChatWorkspace({ onKnowledge, viewerId, workspace, conversationId, onConversationSelected }: { onKnowledge: () => void; viewerId: string; workspace: Doc<"workspaces">; conversationId: Id<"conversations"> | null; onConversationSelected: (id: Id<"conversations">) => void }) {
...
function KnowledgeWorkspace({ onAsk, viewerId, workspace }: { onAsk: () => void; viewerId: string; workspace: Doc<"workspaces"> }) {
  ...
  useEffect(() => { setItems((current) => current.map((item) => documents.some((document) => document.originalName === item.file.name && document.status === "ready") ? { ...item, state: "ready" } : item)); }, [documents]);
  ...
  async function upload() {
    ...
    for (const item of ready) {
      try {
        ...
        await fetch("/api/ingestion/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ viewerId, jobId: created.jobId }) });
      } catch (error) {
  ...
  return <main className="min-w-0 flex-1 overflow-y-auto bg-muted/20">
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><Badge variant="outline" className="mb-3 border-blue-200 bg-blue-50 text-blue-700">Convex Storage intake</Badge><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Submit source material</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Files are stored in Convex Storage and create a durable ingestion job for Hermes. They remain non-authoritative until the job and owner review complete.</p></div><div className="flex gap-3"><Card className="gap-1 px-4 py-3 shadow-none"><p className="text-2xl font-semibold">{documents.filter((document) => document.status === "ready").length}</p><p className="text-xs text-muted-foreground">Ready sources</p></Card><Card className="gap-1 px-4 py-3 shadow-none"><p className="text-2xl font-semibold">{jobs.filter((job) => job.state === "queued" || job.state === "running").length}</p><p className="text-xs text-muted-foreground">Ingestion jobs</p></Card></div></div>
...
export function App() {
  ...
  return <TooltipProvider><div className="flex h-dvh overflow-hidden bg-[#f7f8fa]"><Sidebar {...sidebarProps} />{view === "ask" ? <ChatWorkspace viewerId={viewerId} workspace={activeWorkspace} conversationId={conversationId} onConversationSelected={setConversationId} onKnowledge={() => setView("knowledge")} /> : <KnowledgeWorkspace viewerId={viewerId} workspace={activeWorkspace} onAsk={() => setView("ask")} />}</div></TooltipProvider>;
}
